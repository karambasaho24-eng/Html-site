...
#   .\deployer.ps1
#
# Le projet Netlify n'est relie a aucun depot : rien ne part tout seul quand on
# pousse. Plutot que de recopier deux commandes dont l'une tient sur quinze
# lignes, on les range ici.
#
# TOUT CE FICHIER EST EN ASCII, SANS ACCENT NI TIRET LONG, ET C'EST VOLONTAIRE.
# PowerShell 5 lit les .ps1 en ANSI, pas en UTF-8 : le moindre caractere accentue
# arrive casse et fait echouer l'analyse du script sur une erreur incomprehensible
# ("Le terminateur ' est manquant"). Un script qui ne demarre pas ne sert a rien,
# fut-il mieux ecrit. Ne pas remettre d'accents ici.
#
# Le jeton de publication ne figure PAS dans ce fichier et ne doit jamais y
# figurer : le depot est public, et quiconque le lirait pourrait deposer ce
# qu'il veut sur le site. Il se met une seule fois dans jeton-netlify.txt,
# reste a cote et ignore par git.
# ---------------------------------------------------------------------------
...
# fausse est pire qu'une erreur, car elle fait croire que le site est a jour.
if ($LASTEXITCODE -ne 0) {
  ...
  Write-Host "Ils ne durent que quelques heures. Demandez-en un neuf, puis"
